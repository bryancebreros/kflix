...
export async function insertStats(
  ...
  const operationsDoc = `
    mutation insertStats($favorited: Boolean!, $userId: 
...
export async function updateStats(
  ...
  const operationsDoc = `
mutation updateStats($favorited: Boolean!, $userId: String!, $watched: Boolean!, $videoId: String!) {
  update_stats(
...
export async function getFavorites(userId, token){
  const operationsDoc = `
  query favoriteVideos($userId: String!) {
    stats(where: {
      userId: {_eq: $userId}, 
      favorited: {_eq: true}
    }) {
